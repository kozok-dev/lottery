#include "../common.c"
#define NUMMAX 9999
#define ITEMNAME "合計  大小         奇数偶数     隣数差  " \
				"T ﾚ ﾋ ﾄ 平回  当選間隔                 過去当選コ\n"
#define COLNAME "数1\t数2\t数3\t合計\t大小\t奇偶\t差\t差T\t" \
				"T\tﾚ\tﾋ\tﾄ\t平\t平T\t当間\t当間s\t当間a\tﾎｯﾄ\t無\tM\tB\tS\n"
#define SEPAL "-----------------------------------------------------------------------------------------------------------------------------\n"

char wn[NUMMAX][3], hitmmax[3], hitbmax[3], hitsmax[3];
short wncnt, wnum_cnt1[30], wnum_cnt2[30], wnum_cnt3[30], wnum_cnt4[30], hmmax = -1, hbmax = -1, hsmax = -1;
short sum_cnt[28], big_cnt[4], odd_cnt[4], seq_cnt[4], samepast_cnt[4], avg_cnt[4], sub_cnt[4], num_cnt[30], hm_cnt, hb_cnt, hs_cnt;

struct {
	char en[30], encnt, hit, *wn;
	short cnt, gap;
...
void sub_number(const char *num, char *s)
{
	...
	for (i = 0; i < 2; i++) s[i] = num[i + 1] - num[i];
	s[2] = (s[0] == s[1]) * 2;
}
...
{
	if (num[0] != num[1] && num[0] != num[2] && num[1] != num[2]) return 's';
	if (num[0] == num[1] && num[0] == num[2]) return 'a';
	return 'w';
}
...
char seq_number(const char *num)
{
	...
	while (i < 2) {
		for (j = i + 1; j < 3 && num[i] == num[j] - j + i; j++) cnt += i < j - 1 ? 1 : 2;
		i = j;
	}
	return cnt;
}
...
{
	short wnumcnt[30], wnumavg, i, j;

	for (i = 0; i < 30; i++) wnumcnt[i] = 0;
	for (i = 0; i < g; i++)
		for (j = 0; j < 3; j++)
			wnumcnt[wn[i][j] + j * 10]++;
...
void hot_number(const char *num, char *g1, char *g2, short *s, short *t, short g)
{
	...
	for (i = 0; i < 3; i++) {
		...
		if ((j = g - 10) < 0)
			...
			for (; j < g; j++) {
				for (k = 0; k < 3 && num[i] + i * 10 != wn[j][k] + k * 10; k++);
				if (k == 3) continue;
				g1[3]++;
				break;
			}
...
void past_winning_number(const char *num, short *h, short g)
{
	...
	for (i = wncnt - g; i < wncnt; i++) {
		for (j = 0; j < 3 && num[j] + j * 10 == wn[i][j] + j * 10; j++);
		if (j < 3) {
			for (j = 0; j < 3; j++) ch[j] = 0;
			j = 0;
			for (k = 0; k < 3; k++)
				for (l = 0; l < 3; l++) {
			...
			if (j < 3) {
				for (j = 1; j < 3 && num[j] + j * 10 == wn[i][j] + j * 10; j++);
				h[j == 3]++;	/* 0:ハズレ、1:ミニ。*/
			} else
				h[2]++;	/* ボックス。*/
		} else
...
void write_data(const char *num, FILE *f1, FILE *f2, short flag)
{
	...
	fprintf(f1, "[%d|%d|%d] ", num[0], num[1], num[2]);
	if (f2 != NULL) fprintf(f2, "%d\t%d\t%d\t", num[0], num[1], num[2]);

	i = sum_number(num);
	if (flag == -2) sum_cnt[i]++;
	fprintf(f1, "%2d%+3d ", i, i - 13);
	...
	if (flag == -2) big_cnt[i]++;
	k = k < 4 ? (k + 1) * 3 : 15;
	fprintf(f1, "%d:%d %2d/%2d%+3d ", i, 3 - i, j, k - j, j - k + j);
	if (f2 != NULL) fprintf(f2, "s%d:%d\t", i, 3 - i);
	...
	sub_number(num, a);
	if (flag == -2) sub_cnt[a[2]]++;
	fprintf(f1, "%+d.%+d:%d ", a[0], a[1], a[2]);
	if (f2 != NULL) fprintf(f2, "s%+d.%+d\t%d\t", a[0], a[1], a[2]);
	...
	if (flag == -2) avg_cnt[a[3]]++;
	fprintf(f1, "%c%c%c:%d ", a[0], a[1], a[2], a[3]);
	if (f2 != NULL) fprintf(f2, "%c%c%c\t%d\t", a[0], a[1], a[2], a[3]);

	hot_number(num, a, NULL, &i, NULL, flag < 0 ? wncnt : flag);
	fprintf(f1, "%2d+%2d+%2d=%3d %9f %d ", a[0], a[1], a[2], i, (float)i / 3, a[3]);
	if (f2 != NULL) fprintf(f2, "s%2d+%2d+%2d\t%d\t%f\t%d\t", a[0], a[1], a[2], i, (float)i / 3, a[3]);
	...
	fprintf(f1, "(%d %3d %2d %2d)\n", h[0], h[1], h[2], h[3]);
	if (f2 != NULL) fprintf(f2, "%d\t%d\t%d\t%d\n", h[0], h[1], h[2], h[3]);

	if (flag == -2) {
		for (i = 0; i < 3; i++) num_cnt[num[i] + i * 10]++;

		if (h[1] > hmmax) {
			for (i = 0; i < 3; i++) hitmmax[i] = num[i];
			hmmax = h[1];
		}
		if (h[2] > hbmax) {
			for (i = 0; i < 3; i++) hitbmax[i] = num[i];
			hbmax = h[2];
		}
		if (h[3] > hsmax) {
			for (i = 0; i < 3; i++) hitsmax[i] = num[i];
			hsmax = h[3];
		}
	}
}
...
void main(int argc, char **argv)
{
	...
	char en[NUMMAX][3], num[3], numcnt[3][30], *numrnd;
	char cht1[30], cht2[30], cht3[30], cht4[30], cht5[30], cht6[60], cht7[30], *cp;
	char chtm1[30], chtm3[30], chtm4[30], chtm7[30], *cmp;
	short chth1[4], chth2[4], chth3[4], chth4[4], chth5[4], chth6[4], chth7[4], chtmh1[4], chtmh3[4], chtmh4[4], chtmh7[4];
	short a1[37], a2[37], actwncnt, anatype = argc > 1 ? atoi(argv[1]) : 0, anacnt = argc > 2 ? atoi(argv[2]) : 13, anahit = argc > 3 ? atoi(argv[3]) : 3;
	...
	float wnumavg, chtp1, chtp2, chtp3, chtp4, chtp5, chtp6, chtp7, chtmp1, chtmp3, chtmp4, chtmp7, f, chtp[30];
	float chta1, chta2, chta3, chta4, chta5, chta6, chta7, chtma1, chtma3, chtma4, chtma7;
	FILE *f1, *f2 = NULL;

	/* 当選データ読み込み。*/
	if ((f1 = fopen("wn.txt", "r")) == NULL) return;
	for (wncnt = 0; wncnt < NUMMAX && get_number(f1, wn[wncnt], 3, 3, 0, 9, 1, 0, 0); wncnt++)
		for (i = 0; i < 3; i++) wnum_cnt1[wn[wncnt][i] + i * 10]++;
	fclose(f1);
	...
	if (anatype > 0) {
		...
		if ((f1 = fopen("exp.txt", "r")) != NULL) {
			for (; i < NUMMAX && get_number(f1, en[i], 3, 3, 0, 9, 1, 0, 0); i++);
			fclose(f1);
		}
		...
		if (g_ec > 0) {
			...
			fprintf(f1, "数: %d\n\n[合計]\n", o);
			for (i = 0; i < 28; i++)
				if (sum_cnt[i] > 0) fprintf(f1, "%2d %2d %9f%%\n", i, sum_cnt[i], (float)sum_cnt[i] / (float)o * 100);
			fprintf(f1, "\n[大小]\n");
			for (i = 0; i < 4; i++)
				if (big_cnt[i] > 0) fprintf(f1, "%d:%d %3d %9f%%\n", i, 3 - i, big_cnt[i], (float)big_cnt[i] / (float)o * 100);
			fprintf(f1, "\n[奇数偶数]\n");
			for (i = 0; i < 4; i++)
				if (odd_cnt[i] > 0) fprintf(f1, "%d:%d %3d %9f%%\n", i, 3 - i, odd_cnt[i], (float)odd_cnt[i] / (float)o * 100);
			fprintf(f1, "\n[隣接数字差]\n");
			for (i = 0; i < 4; i++)
				if (sub_cnt[i] > 0) fprintf(f1, "%d %3d %9f%%\n", i, sub_cnt[i], (float)sub_cnt[i] / (float)o * 100);
			fprintf(f1, "\n[連番]\n");
			for (i = 0; i < 4; i++)
				if (seq_cnt[i] > 0) fprintf(f1, "%d %3d %9f%%\n", i, seq_cnt[i], (float)seq_cnt[i] / (float)o * 100);
			fprintf(f1, "\n[ヒキ]\n");
			for (i = 0; i < 4; i++)
				if (samepast_cnt[i] > 0) fprintf(f1, "%d %3d %9f%%\n", i, samepast_cnt[i], (float)samepast_cnt[i] / (float)o * 100);
			fprintf(f1, "\n[平均出現回数]\n");
			for (i = 0; i < 4; i++)
				if (avg_cnt[i] > 0) fprintf(f1, "%d %3d %9f%%\n", i, avg_cnt[i], (float)avg_cnt[i] / (float)o * 100);
			fprintf(f1, "\n[過去当選]\n");
			fprintf(f1, "M %4d %10f%%\n", hm_cnt, (float)hm_cnt / (float)o * 100);
			fprintf(f1, "B %4d %10f%%\n", hb_cnt, (float)hb_cnt / (float)o * 100);
			fprintf(f1, "S %4d %10f%%\n", hs_cnt, (float)hs_cnt / (float)o * 100);
			fprintf(f1, "\n[数字]\n");
			for (i = 0; i < 30; i++)
				if (num_cnt[i] > 0) fprintf(f1, "%s %3d %9f%%\n", numstr[i], num_cnt[i], (float)num_cnt[i] / (float)o * 100);
			fprintf(f1, SEPAL);

			if (g_arf && (f2 = fopen("ana_mm.txt", "w")) != NULL) {
				write_data(hitmmax, f2, NULL, -1);
	...
	for (i = 0; i < 4; i++) chth1[i] = chth2[i] = chth3[i] = chth4[i] = chth5[i] = chth6[i] = chth7[i] = chtmh1[i] = chtmh3[i] = chtmh4[i] = chtmh7[i] = 0;
	r = 0;
	for (wncnt = anacnt < 0 ? 50 : actwncnt - anacnt; wncnt <= actwncnt - (anatype > 0); wncnt++) {
		printf("\b\b\b\b\b%d", wncnt);
		wnumavg = (float)wncnt / 10;
		if (wncnt < actwncnt) r++;
		for (i = 0; i < 30; i++) wnum_cnt1[i] = 0;
		for (i = 0; i < wncnt; i++)
			for (j = 0; j < 3; j++) wnum_cnt1[wn[i][j] + j * 10]++;
		...
		*cmp = 0;
		...
		if (wncnt == actwncnt) {
			fprintf(f1, "最後当選\n    |");
			for (i = 0; i < 30; i++) fprintf(f1, "%s|", numstr[i]);
			fprintf(f1, "\n");
		}
		...
		*cp = 0;
		...
		for (i = 0; i < 30; i++) wnum_cnt2[i] = wnum_cnt3[i] = wnum_cnt4[i] = 0;
		cp = cht3;
		cmp = chtm3;
		for (i = 0; i < 3; i++) {
		...
		for (i = 0; i < 30; i++) {
			...
			if (wncnt == actwncnt) fprintf(f1, "%s: %4d %f%% | %2d %f%% | %d %f%% | %d %9f%% | %2d\n", numstr[i],
				wnum_cnt1[i], (float)wnum_cnt1[i] / ((float)wncnt * 3) * 100,	/* 全ての回数。*/
				wnum_cnt2[i], (float)wnum_cnt2[i] / (50 * 3) * 100,	/* 直前50回数。*/
				wnum_cnt3[i], (float)wnum_cnt3[i] / (25 * 3) * 100,	/* 直前25回数。*/
				wnum_cnt4[i], (float)wnum_cnt4[i] / (10 * 3) * 100,	/* 直前10回数。*/
				j);	/* ハズレ。*/

			if ((float)wnum_cnt1[i] / ((float)wncnt * 3) * 100 > (float)wnum_cnt4[i] / (10 * 3) * 100 && j < 12)
				*cp++ = i;
			else if ((float)wnum_cnt1[i] / ((float)wncnt * 3) * 100 < (float)wnum_cnt2[i] / (50 * 3) * 100)
				*cmp++ = i;
		}
		...
		*cmp = 0;
		...
		if (wncnt == actwncnt) {
			fprintf(f1, "AVG:   ", i);
			for (i = 0; i < 37; i++) fprintf(f1, "|%2d", (int)((float)a2[i] / 30 + 0.5));
			fprintf(f1, "\n");
		}
		...
		*cmp = 0;
		...
		for (k = 0; k < 3; k++) {
			...
			l = m = n = 0;
			for (o = 0; o < wncnt; o++) {
				for (p = 0; p < 3 && wn[o][p] != wn[wncnt - 1][k]; p++);
				if (p < 3) {
					if (l) {	/* 多重当選？ */
						a1[l < 8 ? l : 7]++;
						m++;
		...
		*cp = 0;
		...
		for (i = wncnt - 5; i < wncnt; i++)
			for (j = 0; j < 3; j++) {
				k = 0;
				for (l = i - 1; l >= 0; l--) {
					for (m = 0; m < 3 && wn[i][j] + j * 10 != wn[l][m] + m * 10; m++);
					if (m < 3) break;
					k++;
				}
				a1[k < 36 ? k : 36]++;
		...
		for (k = 0; k < 30; k++) {
			...
			if (wncnt == actwncnt) fprintf(f1, "%s: %4d | %3d %f%% | %3d %f%%\n", numstr[k], wnum_cnt1[k],
		...
		if (wncnt == actwncnt) fprintf(f1, "AVG: %4d | %3d %f%% | %3d %f%%\n", (int)(wnumavg + 0.5),
		...
		*cp = 0;
		...
		for (i = 0; i < 3; i++) {
			...
			for (j = 0; j < 30; j++) numcnt[i][j] = 0;
			a1[i] = 0;

			for (j = 0; j < wncnt - 1; j++)
				for (k = 0; k < 3; k++) {
					if (wn[wncnt - 1][i] + i * 10 != wn[j][k] + k * 10) continue;
					for (l = 0; l < 3; l++) numcnt[i][wn[j + 1][l] + l * 10]++;
					break;
				}
		}
		...
		i = 0;
		for (j = 0; j < 3; j++) {
			if (wncnt == actwncnt) fprintf(f1, "|%2d", (int)((float)a1[j] / 30 + 0.5));
			i += a1[j];
		}
		i = (float)i / 30 + 0.5;
	...
	if (anatype < 2) {
		...
		cp = cht1;
		fprintf(f1, "\n統計一覧 予 %4d %4d %4d %3d %9f %9f%%",
			chth1[0], chth1[1], chth1[2], chth1[3], chta1, chtp1);
		if (anatype <= 0) {
			fprintf(f1, ":");
			for (;;) {
				fprintf(f1, " %s", numstr[*cp++]);
		...
		cmp = chtm1;
		fprintf(f1, "\n         削 %4d %4d %4d %3d %9f %9f%%",
			chtmh1[0], chtmh1[1], chtmh1[2], chtmh1[3], chtma1, chtmp1);
		if (anatype <= 0) {
			fprintf(f1, ":");
			for (;;) {
				fprintf(f1, " %s", numstr[*cmp++]);
		...
		cp = cht2;
		fprintf(f1, "\n最後当選 予 %4d %4d %4d %3d %9f %9f%%",
			chth2[0], chth2[1], chth2[2], chth2[3], chta2, chtp2);
		if (anatype <= 0) {
			fprintf(f1, ":");
			for (;;) {
				fprintf(f1, " %s", numstr[*cp++]);
		...
		cp = cht3;
		fprintf(f1, "\n当選期間 予 %4d %4d %4d %3d %9f %9f%%",
			chth3[0], chth3[1], chth3[2], chth3[3], chta3, chtp3);
		if (anatype <= 0) {
			fprintf(f1, ":");
			for (;;) {
				fprintf(f1, " %s", numstr[*cp++]);
		...
		cmp = chtm3;
		fprintf(f1, "\n         削 %4d %4d %4d %3d %9f %9f%%",
			chtmh3[0], chtmh3[1], chtmh3[2], chtmh3[3], chtma3, chtmp3);
		if (anatype <= 0) {
			fprintf(f1, ":");
			for (;;) {
				fprintf(f1, " %s", numstr[*cmp++]);
		...
		cp = cht4;
		fprintf(f1, "\n飛後当選 予 %4d %4d %4d %3d %9f %9f%%",
			chth4[0], chth4[1], chth4[2], chth4[3], chta4, chtp4);
		if (anatype <= 0) {
			fprintf(f1, ":");
			for (;;) {
				fprintf(f1, " %s", numstr[*cp++]);
		...
		cmp = chtm4;
		fprintf(f1, "\n         削 %4d %4d %4d %3d %9f %9f%%",
			chtmh4[0], chtmh4[1], chtmh4[2], chtmh4[3], chtma4, chtmp4);
		if (anatype <= 0) {
			fprintf(f1, ":");
			for (;;) {
				fprintf(f1, " %s", numstr[*cmp++]);
		...
		cp = cht5;
		fprintf(f1, "\n多重当選 予 %4d %4d %4d %3d %9f %9f%%",
			chth5[0], chth5[1], chth5[2], chth5[3], chta5, chtp5);
		if (anatype <= 0) {
			fprintf(f1, ":");
			for (;;) {
		...
		cp = cht6;
		fprintf(f1, "\n前後数当 予 %4d %4d %4d %3d %9f %9f%%",
			chth6[0], chth6[1], chth6[2], chth6[3], chta6, chtp6);
		if (anatype <= 0) {
			fprintf(f1, ":");
			for (;;) {
		...
		cp = cht7;
		fprintf(f1, "\n後追い数 予 %4d %4d %4d %3d %9f %9f%%",
			chth7[0], chth7[1], chth7[2], chth7[3], chta7, chtp7);
		if (anatype <= 0) {
			fprintf(f1, ":");
			for (;;) {
				fprintf(f1, " %s", numstr[*cp++]);
		...
		cmp = chtm7;
		fprintf(f1, "\n         削 %4d %4d %4d %3d %9f %9f%%",
			chtmh7[0], chtmh7[1], chtmh7[2], chtmh7[3], chtma7, chtmp7);
		if (anatype <= 0) {
			fprintf(f1, ":");
			for (;;) {
				fprintf(f1, " %s", numstr[*cmp++]);
	...
	if (anatype <= 0) {
		...
		for (i = 0; i < 30; i++) {
			...
			for (cp = cht5; *cp; cp++) {
				if (i != *cp && i + 10 != *cp && i + 20 != *cp) continue;
				f += chtp5;
				break;
			}
			for (cp = cht6; *cp; cp++) {
	...
	if (anatype == 2) {
		...
		for (i = 0; i < 11; i++) {
			...
			for (cht1hh = chthh[i]; cht1hh->cnt > 0; cht1hh++) {
				fprintf(f1, "%4d [%d|%d|%d] %3d %dh %2d:", cht1hh->cnt, cht1hh->wn[0], cht1hh->wn[1],
				cht1hh->wn[2], cht1hh->gap, cht1hh->hit, cht1hh->encnt);
