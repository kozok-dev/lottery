...
char g_encnt;
HWND g_hwwn[6], g_hwen[30][6], g_hwttl;
HFONT g_hfont;
...
LRESULT CALLBACK proc_edit(HWND hwnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_KILLFOCUS:
		...
		i = atoi(buf1);
		if (i < 1 || i > 31) SetWindowText(hwnd, NULL);
	...
	case WM_KEYUP:
		for (i = 0; i < g_encnt; i++)
			for (j = 0; j < 6; j++) RedrawWindow(g_hwen[i][j], NULL, NULL, RDW_INVALIDATE);
		print_hit_stat();
		break;
	}
...
LRESULT CALLBACK proc_static1(HWND hwnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_PAINT:
		...
		SetTextColor(hdc, i < 5 ? RGB(255, 0, 0) : i < 6 ? RGB(0, 0, 255) : RGB(0, 0, 0));
...
LRESULT CALLBACK proc_static2(HWND hwnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_PAINT:
		...
		SetTextColor(hdc, hit < 3 ? RGB(0, 0, 0) : hit < 6 ? RGB(255, 0, 0) : RGB(0, 0, 255));
		wsprintf(buf1, TEXT("%d"), hit < 6 ? hit : hit - 2);
		GetClientRect(hwnd, &rect);
...
int WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpCmdLine, int nCmdShow)
{
	...
	if ((g_hwwn[5] = CreateWindowEx(WS_EX_CLIENTEDGE, TEXT("EDIT"), NULL, WS_CHILD | WS_TABSTOP | WS_VISIBLE | ES_NUMBER,
	138, 10, 22, 19, hwnd, NULL, hInstance, NULL)) == NULL) return 0;
	SendMessage(g_hwwn[5], WM_SETFONT, (WPARAM)g_hfont, FALSE);
	SetWindowLong(g_hwwn[5], GWL_WNDPROC, (int)proc_edit);

	if ((hwbtn = CreateWindow(TEXT("BUTTON"), TEXT("Sort"), WS_CHILD | WS_TABSTOP | WS_VISIBLE,
	173, 10, 40, 19, hwnd, (HMENU)1, hInstance, NULL)) == NULL) return 0;
	SendMessage(hwbtn, WM_SETFONT, (WPARAM)g_hfont, FALSE);

	i = 0;
	if ((f = _tfopen(TEXT("exp.txt"), TEXT("r"))) != NULL) {
		for (; i < 30 && get_number(f, num, 5, 5, 1, 31, 0, 5, 0); i++) {
			for (j = 0; j < 5; j++) {
			...
			if ((g_hwen[i][5] = CreateWindow(TEXT("STATIC"), NULL, WS_CHILD | WS_VISIBLE,
			134, i * 20 + 51, 10, 15, hwnd, NULL, hInstance, NULL)) == NULL) return 0;
			SetProp(g_hwen[i][5], TEXT("IDX"), (HANDLE)i);
			SetWindowLong(g_hwen[i][5], GWL_WNDPROC, (int)proc_static2);
		}
		fclose(f);
	}
	g_encnt = i;
	...
	171, 50, 130, 110, hwnd, NULL, hInstance, NULL)) == NULL) return 0;
