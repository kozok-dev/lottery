	...
	case WM_COMMAND:
		...
		case 1:
			...
			case LBN_SELCHANGE:
				...
				_stscanf(buf, TEXT("%d|%d|%d|%d|%d"),
				&g_num[0], &g_num[1], &g_num[2], &g_num[3], &g_num[4]);

				for (i = 0; i < 6; i++) hit[i] = 0;
				for (i = 1; i < 9; i++) {
					j = 0;
					for (k = i; k < 32; k += 10)
						for (l = 0; l < 5; l++)
				...
				for (i = 0; i < 31; i++) RedrawWindow(g_hnum[i], NULL, NULL, RDW_INVALIDATE);
				break;
			}
			break;
		}
...
int WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpCmdLine, int nCmdShow)
{
	...
	200, 100, 140, 430, NULL, NULL, hInstance, NULL)) == NULL) return 0;
	SendMessage(hwnd, WM_SETFONT, (WPARAM)g_hfont, FALSE);

	if ((g_hlb = CreateWindowEx(WS_EX_CLIENTEDGE, TEXT("LISTBOX"), NULL,
	WS_CHILD | WS_VISIBLE | WS_VSCROLL | LBS_NOTIFY, 5, 5, 125, 80,
	...
	5, 80, 30, 80, hwnd, NULL, hInstance, NULL)) == NULL) return 0;
	...
	if ((f = fopen(strlen(lpCmdLine) > 0 ? lpCmdLine : "exp.txt", "r")) != NULL) {
		while (get_number(f, num, 5, 5, 1, 31, 0, 5, 0)) {
			wsprintf(buf, TEXT("%2d|%2d|%2d|%2d|%2d"), num[0], num[1], num[2], num[3], num[4]);
	...
	5, 305, 125, 95, hwnd, NULL, hInstance, NULL)) == NULL) return 0;
