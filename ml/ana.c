...
#define ITEMNAME "合計    大小         奇数偶数     数字集団            下一桁       同番台 " \
	"隣接数字差    ｾｯﾄ球   ﾚ ﾋ ﾄ 平回    当選間隔                      過去当選コ\n"
#define COLNAME "数1\t数2\t数3\t数4\t数5\t合計\t大小\t奇偶\t集1\t集1T\t集2\t集2T\t" \
				"下\t下T\t番台\t番台T\t差\t差T\tｾｯﾄ\tｾｯﾄT\tﾚ\tﾋ\tﾄ\t平\t平T\t" \
				"当間\t当間s\t当間a\tﾎｯﾄ\th0\th1\th2\th3\th4\th4b\th5\n"
#define SEPAL "--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------\n"

char wn[NUMMAX][6], hit0max[5], hit0min[5], hit1max[5], hit1min[5], hit2max[5], hit2min[5], hit3max[5], hit4max[5], hit5max[5];
short wncnt, wnum_cnt1[32], wnum_cnt2[32], wnum_cnt3[32], wnum_cnt4[32], h0max = -1, h0min = 999, h1max = -1, h1min = 999, h2max = -1, h2min = 999, h3max = -1, h4max = -1, h5max = -1;
int sum_cnt[146], big_cnt[6], odd_cnt[6], seq_cnt[6], samepast_cnt[6], avg_cnt[6], sub_cnt[6], same1_cnt[6], same2_cnt[6], group_cnt[11], num_cnt[32], h4_cnt, h4b_cnt, h5_cnt;

struct {
	char en[32], encnt, hit, *wn;
	short cnt, gap;
...
{
	char g[2], ch[5], chf, i, j, k = 0;

	for (i = 0; i < 5; i++) {
		if (t)
			g1[i] = (num[i] - 1) % 6 + 'A';	/* セット球。*/
		else
			/* 6分割。*/
			if (num[i] < 6) g1[i] = 'A';
			else if (num[i] < 11) g1[i] = 'B';
			else if (num[i] < 16) g1[i] = 'C';
			else if (num[i] < 21) g1[i] = 'D';
			else if (num[i] < 27) g1[i] = 'E';
			else g1[i] = 'F';
	...
	if (g[0] == 2 && g[1] == 0) g1[5] = 'b';	/* 例：AABCD */
	else if (g[0] == 2 && g[1] == 2) g1[5] = 'c';	/* 例：AABBC */
	else if (g[0] == 3 && g[1] == 0) g1[5] = 'd';	/* 例：AAABC */
	else if (g[0] == 3 && g[1] == 2) g1[5] = 'e';	/* 例：AAABB */
	else if (g[0] == 4) g1[5] = 'f';	/* 例：AAAAB */
	else if (g[0] == 5) g1[5] = 'g';	/* 例：AAAAA */
	else g1[5] = 'a';	/* 例：ABCDEF */

	/* 9分割。*/
	for (i = 0; i < 9; i++) g2[i] = 0;
	for (i = 0; i < 5; i++)
		if (num[i] < 6) g2[0]++;
		else if (num[i] < 9) g2[1]++;
		else if (num[i] < 13) g2[2]++;
		else if (num[i] < 16) g2[3]++;
		else if (num[i] < 19) g2[4]++;
		else if (num[i] < 22) g2[5]++;
		else if (num[i] < 25) g2[6]++;
		else if (num[i] < 28) g2[7]++;
		else g2[8]++;
	g2[9] = 0;
	for (i = 0; i < 9; i++)
		if (g2[i] > 1) g2[9] += g2[i];
...
{
	char ch1[5], ch2[5], chf1, chf2, i, j, k, l, m, n;

	/* 下1桁。*/
	for (i = 0; i < 11; i++) s1[i] = 0;
	for (i = 0; i < 5; i++) s1[num[i] % 10]++;

	/* 同番台。*/
	for (i = 0; i < 5; i++) s2[i] = ch1[i] = ch2[i] = 0;
	for (i = 0; i < 5; i++) s2[num[i] / 10]++;
...
char seq_number(const char *num)
{
	...
	while (i < 4) {
		for (j = i + 1; j < 5 && num[i] >= num[j] - j + i; j++) cnt += i < j - 1 ? 1 : 2;
		i = j;
	}
	return cnt;
}
...
{
	short wnumcnt[32], wnumavg, i, j;

	for (i = 1; i < 32; i++) wnumcnt[i] = 0;
	for (i = 0; i < g; i++)
		for (j = 0; j < 5; j++) wnumcnt[wn[i][j]]++;
	wnumavg = (float)g * 5 / 31 + 0.5;
...
char mark_number(const char *num)
{
	...
	if (m[0] > 3 || m[1] > 5 || m[2] > 4 || m[3] > 2 || m[4] > 0 || m[5] > 0) return 1;
	...
	for (i = 0; i < 3; i++) {
		...
		for (l = i + 1; l < 5; l++) {
			...
			if (num[i] < 14 && num[i] % 10 != 1 && num[i] % 10 != 2) {
...
void write_data(const char *num, FILE *f1, FILE *f2, short flag)
{
	...
	if (g_ec != 1 || flag != -2)
		fprintf(f1, "[%2d|%2d|%2d|%2d|%2d] ", num[0], num[1], num[2], num[3], num[4]);
	if (f2 != NULL) fprintf(f2, "%d\t%d\t%d\t%d\t%d\t", num[0], num[1], num[2], num[3], num[4]);

	i = sum_number(num);
	if (flag == -2) sum_cnt[i]++;
	if (g_ec != 1 || flag != -2) fprintf(f1, "%3d%+4d ", i, i - 80);
	...
	if (g_ec != 1 || flag != -2) {
		k = k < 4 ? (k + 1) * 5 : 25;
		fprintf(f1, "%d:%d %2d/%2d%+3d ", i, 5 - i, j, k - j, j - k + j);
	}
	if (f2 != NULL) fprintf(f2, "s%d:%d\t", i, 5 - i);
	...
	if (flag == -2) group_cnt[a1[5] - 'a']++;
	if (g_ec != 1 || flag != -2)
		fprintf(f1, "%c%c%c%c%c:%c-%d%d%d%d%d%d%d%d%d:%d ",
		a1[0], a1[1], a1[2], a1[3], a1[4], a1[5],
		a2[0], a2[1], a2[2], a2[3], a2[4], a2[5], a2[6], a2[7], a2[8], a2[9]);
	if (f2 != NULL) fprintf(f2, "%c%c%c%c%c\t%c\ts%d%d%d%d%d%d%d%d%d\t%d\t",
	a1[0], a1[1], a1[2], a1[3], a1[4], a1[5],
	a2[0], a2[1], a2[2], a2[3], a2[4], a2[5], a2[6], a2[7], a2[8], a2[9]);
	...
	if (g_ec != 1 || flag != -2)
		fprintf(f1, "%d%d%d%d%d%d%d%d%d%d:%d %d%d%d%d:%d ",
		a1[0], a1[1], a1[2], a1[3], a1[4], a1[5], a1[6], a1[7], a1[8], a1[9], a1[10],
		a2[0], a2[1], a2[2], a2[3], a2[4]);
	if (f2 != NULL) fprintf(f2, "s%d%d%d%d%d%d%d%d%d%d\t%d\ts%d%d%d%d\t%d\t",
	a1[0], a1[1], a1[2], a1[3], a1[4], a1[5], a1[6], a1[7], a1[8], a1[9], a1[10],
	a2[0], a2[1], a2[2], a2[3], a2[4]);

	sub_number(num, a1);
	if (flag == -2) sub_cnt[a1[4]]++;
	if (g_ec != 1 || flag != -2)
		fprintf(f1, "%2d-%2d-%2d-%2d:%d ", a1[0], a1[1], a1[2], a1[3], a1[4]);
	if (f2 != NULL) fprintf(f2, "s%2d-%2d-%2d-%2d\t%d\t", a1[0], a1[1], a1[2], a1[3], a1[4]);

	group_number(num, a1, a2, 1);
	if (g_ec != 1 || flag != -2)
		fprintf(f1, "%c%c%c%c%c:%c ", a1[0], a1[1], a1[2], a1[3], a1[4], a1[5]);
	if (f2 != NULL) fprintf(f2, "%c%c%c%c%c\t%c\t", a1[0], a1[1], a1[2], a1[3], a1[4], a1[5]);
	...
	if (flag == -2) avg_cnt[a1[5]]++;
	if (g_ec != 1 || flag != -2)
		fprintf(f1, "%c%c%c%c%c:%d ", a1[0], a1[1], a1[2], a1[3], a1[4], a1[5]);
	if (f2 != NULL) fprintf(f2, "%c%c%c%c%c\t%d\t", a1[0], a1[1], a1[2], a1[3], a1[4], a1[5]);

	hot_number(num, a1, a2, &i, &j, flag < 0 ? wncnt : flag);
	if (g_ec != 1 || flag != -2)
		fprintf(f1, "%2d+%2d+%2d+%2d+%2d=%2d %9f %d ", a1[0], a1[1], a1[2], a1[3], a1[4], i, (float)i / 5, a1[5]);
	if (f2 != NULL) fprintf(f2, "s%2d+%2d+%2d+%2d+%2d\t%d\t%f\t%d\t", a1[0], a1[1], a1[2], a1[3], a1[4], i, (float)i / 5, a1[5]);
	...
	if (g_ec != 1 || flag != -2)
		fprintf(f1, "(%4d %4d %3d %2d %2d %d %d)\n", h[0], h[1], h[2], h[3], h[4], h[6], h[5]);
	if (f2 != NULL) fprintf(f2, "%d\t%d\t%d\t%d\t%d\t%d\t%d\n", h[0], h[1], h[2], h[3], h[4], h[6], h[5]);

	if (flag == -2) {
		for (i = 0; i < 5; i++) num_cnt[num[i]]++;

		if (h[0] > h0max) {
			for (i = 0; i < 5; i++) hit0max[i] = num[i];
			h0max = h[0];
		}
		if (h[0] < h0min) {
			for (i = 0; i < 5; i++) hit0min[i] = num[i];
			h0min = h[0];
		}
		if (h[1] > h1max) {
			for (i = 0; i < 5; i++) hit1max[i] = num[i];
			h1max = h[1];
		}
		if (h[1] < h1min) {
			for (i = 0; i < 5; i++) hit1min[i] = num[i];
			h1min = h[1];
		}
		if (h[2] > h2max) {
			for (i = 0; i < 5; i++) hit2max[i] = num[i];
			h2max = h[2];
		}
		if (h[2] < h2min) {
			for (i = 0; i < 5; i++) hit2min[i] = num[i];
			h2min = h[2];
		}
		if (h[3] > h3max) {
			for (i = 0; i < 5; i++) hit3max[i] = num[i];
			h3max = h[3];
		}
		if (h[4] > h4max) {
			for (i = 0; i < 5; i++) hit4max[i] = num[i];
			h4max = h[4];
		}
		if (h[5] > h5max) {
			for (i = 0; i < 5; i++) hit5max[i] = num[i];
			h5max = h[5];
		}
	}
}
...
{
	char en[NUMMAX][5], num[5], numcnt[32][32], *numrnd;
	char cht1[32], cht2[32], cht3[32], cht4[32], cht5[32], cht6[32], cht7[32], cht8[32], *cp;
	char chtm1[32], chtm3[32], chtm4[32], chtm7[32], *cmp;
	char alnum[32][12], maxday[] = {31, 28, 31, 30, 31, 30, 31, 31, 30, 31, 30, 31};
	char *alstr[] = {
		"03/21", "04/20", "05/21", "06/21", "07/23", "08/23",
		"09/23", "10/23", "11/22", "12/22", "01/20", "02/19"
	};
	short chth1[6], chth2[6], chth3[6], chth4[6], chth5[6], chth6[6], chth7[6], chth8[6], chtmh1[6], chtmh3[6], chtmh4[6], chtmh7[6];
	short a1[37], a2[37], actwncnt, anatype = argc > 1 ? atoi(argv[1]) : 0, anacnt = argc > 2 ? atoi(argv[2]) : 13, anahit = argc > 3 ? atoi(argv[3]) : 5;
	...
	float wnumavg, chtp1, chtp2, chtp3, chtp4, chtp5, chtp6, chtp7, chtp8, chtmp1, chtmp3, chtmp4, chtmp7, f, chtp[32];
	float chta1, chta2, chta3, chta4, chta5, chta6, chta7, chta8, chtma1, chtma3, chtma4, chtma7;
	FILE *f1, *f2 = NULL;

	/* 当選データ読み込み。*/
	if ((f1 = fopen("wn.txt", "r")) == NULL) return;
	for (wncnt = 0; wncnt < NUMMAX && get_number(f1, wn[wncnt], 6, 6, 1, 31, 0, 5, 0); wncnt++)
		for (i = 0; i < 5; i++) wnum_cnt1[wn[wncnt][i]]++;
	fclose(f1);
	...
	if (anatype > 0) {
		...
		if ((f1 = fopen("exp.txt", "r")) != NULL) {
			for (; i < NUMMAX && get_number(f1, en[i], 5, 5, 1, 31, 0, 5, 0); i++);
			fclose(f1);
		}
		...
		if (g_rndcnt > 0) {
			...
			numrnd = (char *)calloc(wncnt * 5, sizeof(char));
			i = 0;
			for (j = 1; j < 32; j++)
		...
		if (g_ec > 0) {
			...
			fprintf(f1, "数: %d\n\n[合計]\n", o);
			for (i = 15; i < 146; i++)
				if (sum_cnt[i] > 0) fprintf(f1, "%3d %4d %9f%%\n", i, sum_cnt[i], (float)sum_cnt[i] / (float)o * 100);
			fprintf(f1, "\n[大小]\n");
			for (i = 0; i < 6; i++)
				if (big_cnt[i] > 0) fprintf(f1, "%d:%d %5d %9f%%\n", i, 5 - i, big_cnt[i], (float)big_cnt[i] / (float)o * 100);
			fprintf(f1, "\n[奇数偶数]\n");
			for (i = 0; i < 6; i++)
				if (odd_cnt[i] > 0) fprintf(f1, "%d:%d %5d %9f%%\n", i, 5 - i, odd_cnt[i], (float)odd_cnt[i] / (float)o * 100);
			fprintf(f1, "\n[数字集団]\n");
			for (i = 0; i < 11; i++)
				if (group_cnt[i] > 0) fprintf(f1, "%c %5d %9f%%\n", i + 'a', group_cnt[i], (float)group_cnt[i] / (float)o * 100);
			fprintf(f1, "\n[下1桁]\n");
			for (i = 0; i < 6; i++)
				if (same1_cnt[i] > 0) fprintf(f1, "%d %5d %9f%%\n", i, same1_cnt[i], (float)same1_cnt[i] / (float)o * 100);
			fprintf(f1, "\n[同番台]\n");
			for (i = 2; i < 6; i++)
				if (same2_cnt[i] > 0) fprintf(f1, "%d %5d %9f%%\n", i, same2_cnt[i], (float)same2_cnt[i] / (float)o * 100);
			fprintf(f1, "\n[隣接数字差]\n");
			for (i = 0; i < 6; i++)
				if (sub_cnt[i] > 0) fprintf(f1, "%d %5d %9f%%\n", i, sub_cnt[i], (float)sub_cnt[i] / (float)o * 100);
			fprintf(f1, "\n[連番]\n");
			for (i = 0; i < 6; i++)
				if (seq_cnt[i] > 0) fprintf(f1, "%d %5d %9f%%\n", i, seq_cnt[i], (float)seq_cnt[i] / (float)o * 100);
			fprintf(f1, "\n[ヒキ]\n");
			for (i = 0; i < 6; i++)
				if (samepast_cnt[i] > 0) fprintf(f1, "%d %5d %9f%%\n", i, samepast_cnt[i], (float)samepast_cnt[i] / (float)o * 100);
			fprintf(f1, "\n[平均出現回数]\n");
			for (i = 0; i < 6; i++)
				if (avg_cnt[i] > 0) fprintf(f1, "%d %5d %9f%%\n", i, avg_cnt[i], (float)avg_cnt[i] / (float)o * 100);
			fprintf(f1, "\n[過去当選コ]\n");
			fprintf(f1, "4  %5d %9f%%\n", h4_cnt, (float)h4_cnt / (float)o * 100);
			fprintf(f1, "4b %5d %9f%%\n", h4b_cnt, (float)h4b_cnt / (float)o * 100);
			fprintf(f1, "5  %5d %9f%%\n", h5_cnt, (float)h5_cnt / (float)o * 100);
			fprintf(f1, "\n[数字]\n");
			for (i = 1; i < 32; i++)
				if (num_cnt[i] > 0) fprintf(f1, "%2d %5d %9f%%\n", i, num_cnt[i], (float)num_cnt[i] / (float)o * 100);
			fprintf(f1, SEPAL);
	...
	for (i = 0; i < 6; i++) chth1[i] = chth2[i] = chth3[i] = chth4[i] = chth5[i] = chth6[i] = chth7[i] = chth8[i] = chtmh1[i] = chtmh3[i] = chtmh4[i] = chtmh7[i] = 0;
	r = 0;
	for (wncnt = anacnt < 0 ? 50 : actwncnt - anacnt; wncnt <= actwncnt - (anatype > 0); wncnt++) {
		printf("\b\b\b\b%d", wncnt);
		wnumavg = (float)wncnt * 5 / 31;
		if (wncnt < actwncnt) r++;
		for (i = 1; i < 32; i++) wnum_cnt1[i] = 0;
		for (i = 0; i < wncnt; i++)
			for (j = 0; j < 5; j++) wnum_cnt1[wn[i][j]]++;
		...
		for (i = 1; i < 32; i++) {
			...
			if (wncnt == actwncnt) fprintf(f1, "%2d: %2d %f | %2d %2d %f %f | %f %3d %10f%%\n", i,
		...
		*cp = 0;
		...
		if (wncnt < actwncnt) {
			...
			if (anatype == 3 && (i == anahit || wncnt >= actwncnt - 1)) {
				cht1hh->cnt = wncnt + 1;
				cht1hh->wn = wn[wncnt];
				cht1hh->gap = cht1gap;
				cht1hh->hit = i;
		...
		*cmp = 0;
		...
		if (wncnt == actwncnt) {
			fprintf(f1, "最後当選\n    |");
			for (i = 1; i < 32; i++) fprintf(f1, "%2d|", i);
			fprintf(f1, "\n");
		}
		...
		*cp = 0;
		...
		for (i = 1; i < 32; i++) wnum_cnt2[i] = wnum_cnt3[i] = wnum_cnt4[i] = 0;
		cp = cht3;
		cmp = chtm3;
		for (i = 0; i < 5; i++) {
		...
		for (i = 1; i < 32; i++) {
			...
			if (wncnt == actwncnt) fprintf(f1, "%2d: %3d %f%% | %2d %f%% | %2d %f%% | %d %9f%% | %2d\n", i,
				wnum_cnt1[i], (float)wnum_cnt1[i] / ((float)wncnt * 5) * 100,	/* 全ての回数。*/
				wnum_cnt2[i], (float)wnum_cnt2[i] / (50 * 5) * 100,	/* 直前50回数。*/
				wnum_cnt3[i], (float)wnum_cnt3[i] / (25 * 5) * 100,	/* 直前25回数。*/
				wnum_cnt4[i], (float)wnum_cnt4[i] / (10 * 5) * 100,	/* 直前10回数。*/
				j);	/* ハズレ。*/

			if ((float)wnum_cnt1[i] / ((float)wncnt * 5) * 100 > (float)wnum_cnt4[i] / (10 * 5) * 100 &&
			j < 12) *cp++ = i;
			else if ((float)wnum_cnt1[i] / ((float)wncnt * 5) * 100 < (float)wnum_cnt2[i] / (50 * 5) * 100)
				*cmp++ = i;
		}
		...
		*cmp = 0;
		...
		if (wncnt == actwncnt) {
			fprintf(f1, "AG:   ", i);
			for (i = 0; i < 37; i++) fprintf(f1, "|%2d", (int)((float)a2[i] / 31 + 0.5));
			fprintf(f1, "\n");
		}
		...
		*cmp = 0;
		...
		for (k = 0; k < 5; k++) {
			...
			l = m = n = 0;
			for (o = 0; o < wncnt; o++) {
				for (p = 0; p < 5 && wn[o][p] != wn[wncnt - 1][k]; p++);
				if (p < 5) {
					if (l) {	/* 多重当選？ */
						a1[l < 5 ? l : 5]++;
						m++;
		...
		for (k = 1; k < 32; k++) {
			...
			for (n = 1; n < wncnt; n++)
				for (o = 0; o < 5; o++) {
					...
					for (p = 0; p < 5; p++)
						if (wn[n - 1][p] == (k > 1 ? k - 1 : 31)) l++;	/* 数字-1 */
						else if (wn[n - 1][p] == (k < 31 ? k + 1 : 1)) m++;	/* 数字+1 */
					break;
				}
			i += l;
			j += m;
		...
		for (k = 0; k < 5; k++) {
			...
			for (n = 1; n < wncnt; n++)
				for (o = 0; o < 5; o++) {
					...
					for (p = 0; p < 5; p++)
						if (wn[n - 1][p] == (wn[wncnt - 1][k] > 1 ? wn[wncnt - 1][k] - 1 : 31))
							l++;	/* 数字-1 */
						else if (wn[n - 1][p] == (wn[wncnt - 1][k] < 31 ? wn[wncnt - 1][k] + 1 : 1))
							m++;	/* 数字+1 */
					break;
				}

			if ((float)l / (float)wnum_cnt1[wn[wncnt - 1][k]] * 100 < (float)i / wnumavg * 100)
				*cp++ = (wn[wncnt - 1][k] > 1 ? wn[wncnt - 1][k] - 1 : 31);
			if ((float)m / (float)wnum_cnt1[wn[wncnt - 1][k]] * 100 < (float)j / wnumavg * 100)
				*cp++ = (wn[wncnt - 1][k] < 31 ? wn[wncnt - 1][k] + 1 : 1);
		}
		...
		*cp = 0;
		...
		for (i = 0; i < 5; i++) {
			...
			for (j = 1; j < 32; j++) numcnt[i][j] = 0;
			a1[i] = 0;

			for (j = 0; j < wncnt - 1; j++)
				for (k = 0; k < 5; k++) {
					if (wn[wncnt - 1][i] != wn[j][k]) continue;
					for (l = 0; l < 5; l++) numcnt[i][wn[j + 1][l]]++;
					break;
				}
		}
		...
		i = 0;
		for (j = 0; j < 5; j++) {
			if (wncnt == actwncnt) fprintf(f1, "|%2d", (int)((float)a1[j] / 31 + 0.5));
			i += a1[j];
		}
		i = (float)i / 31 + 0.5;
		...
		*cmp = 0;
		...
		for (i = 1; i < 32; i++) {
			...
			for (j = 0; j < wncnt; j++)
				for (k = 0; k < 5; k++) {
					...
					for (l = 0; l < 5; l++) numcnt[i][wn[j][l]]++;
					break;
				}

			if (wncnt == actwncnt) {
				fprintf(f1, "%2d ", i);
				j = (float)wncnt * 0.03 + 0.5;
				for (k = 1; k < 32; k++)
		...
		if (wncnt == actwncnt) {
			...
			for (i = 0; i < 12; i++) {
				j = 0;
				for (k = 1; k < 32; k++) j += alnum[k][i];
				fprintf(f1, "%5.2f|", (float)j / 31);
			}
			fprintf(f1, "\n");
		}

		i = 0;
		for (j = 1; j < 32; j++) i += alnum[j][n];
		i = (float)i / 31 + 0.5;
		cp = cht8;
		for (j = 1; j < 32; j++)
			if (alnum[j][n] > i) *cp++ = j;
	...
	if (anatype < 2) {
		...
		cp = cht1;
		fprintf(f1, "\n統計一覧 予 %4d %3d %3d %3d %3d %2d %9f %9f%%",
			chth1[0], chth1[1], chth1[2], chth1[3], chth1[4], chth1[5], chta1, chtp1);
		if (anatype <= 0) {
			fprintf(f1, ":");
			for (;;) {
				fprintf(f1, " %2d", *cp++);
		...
		cmp = chtm1;
		fprintf(f1, "\n         削 %4d %3d %3d %3d %3d %2d %9f %9f%%",
			chtmh1[0], chtmh1[1], chtmh1[2], chtmh1[3], chtmh1[4], chtmh1[5], chtma1, chtmp1);
		if (anatype <= 0) {
			fprintf(f1, ":");
			for (;;) {
				fprintf(f1, " %2d", *cmp++);
		...
		cp = cht2;
		fprintf(f1, "\n最後当選 予 %4d %3d %3d %3d %3d %2d %9f %9f%%",
			chth2[0], chth2[1], chth2[2], chth2[3], chth2[4], chth2[5], chta2, chtp2);
		if (anatype <= 0) {
			fprintf(f1, ":");
			for (;;) {
				fprintf(f1, " %2d", *cp++);
		...
		cp = cht3;
		fprintf(f1, "\n当選期間 予 %4d %3d %3d %3d %3d %2d %9f %9f%%",
			chth3[0], chth3[1], chth3[2], chth3[3], chth3[4], chth3[5], chta3, chtp3);
		if (anatype <= 0) {
			fprintf(f1, ":");
			for (;;) {
				fprintf(f1, " %2d", *cp++);
		...
		cmp = chtm3;
		fprintf(f1, "\n         削 %4d %3d %3d %3d %3d %2d %9f %9f%%",
			chtmh3[0], chtmh3[1], chtmh3[2], chtmh3[3], chtmh3[4], chtmh3[5], chtma3, chtmp3);
		if (anatype <= 0) {
			fprintf(f1, ":");
			for (;;) {
				fprintf(f1, " %2d", *cmp++);
		...
		cp = cht4;
		fprintf(f1, "\n飛後当選 予 %4d %3d %3d %3d %3d %2d %9f %9f%%",
			chth4[0], chth4[1], chth4[2], chth4[3], chth4[4], chth4[5], chta4, chtp4);
		if (anatype <= 0) {
			fprintf(f1, ":");
			for (;;) {
				fprintf(f1, " %2d", *cp++);
		...
		cmp = chtm4;
		fprintf(f1, "\n         削 %4d %3d %3d %3d %3d %2d %9f %9f%%",
			chtmh4[0], chtmh4[1], chtmh4[2], chtmh4[3], chtmh4[4], chtmh4[5], chtma4, chtmp4);
		if (anatype <= 0) {
			fprintf(f1, ":");
			for (;;) {
				fprintf(f1, " %2d", *cmp++);
		...
		cp = cht5;
		fprintf(f1, "\n多重当選 予 %4d %3d %3d %3d %3d %2d %9f %9f%%",
			chth5[0], chth5[1], chth5[2], chth5[3], chth5[4], chth5[5], chta5, chtp5);
		if (anatype <= 0) {
			fprintf(f1, ":");
			for (;;) {
				fprintf(f1, " %2d", *cp++);
		...
		cp = cht6;
		fprintf(f1, "\n前後数当 予 %4d %3d %3d %3d %3d %2d %9f %9f%%",
			chth6[0], chth6[1], chth6[2], chth6[3], chth6[4], chth6[5], chta6, chtp6);
		if (anatype <= 0) {
			fprintf(f1, ":");
			for (;;) {
		...
		cp = cht7;
		fprintf(f1, "\n後追い数 予 %4d %3d %3d %3d %3d %2d %9f %9f%%",
			chth7[0], chth7[1], chth7[2], chth7[3], chth7[4], chth7[5], chta7, chtp7);
		if (anatype <= 0) {
			fprintf(f1, ":");
			for (;;) {
				fprintf(f1, " %2d", *cp++);
		...
		cmp = chtm7;
		fprintf(f1, "\n         削 %4d %3d %3d %3d %3d %2d %9f %9f%%",
			chtmh7[0], chtmh7[1], chtmh7[2], chtmh7[3], chtmh7[4], chtmh7[5], chtma7, chtmp7);
		if (anatype <= 0) {
			fprintf(f1, ":");
			for (;;) {
				fprintf(f1, " %2d", *cmp++);
		...
		cp = cht8;
		fprintf(f1, "\n占星術   予 %4d %3d %3d %3d %3d %2d %9f %9f%%",
			chth8[0], chth8[1], chth8[2], chth8[3], chth8[4], chth8[5], chta8, chtp8);
		if (anatype <= 0) {
			fprintf(f1, ":");
			for (;;) {
				fprintf(f1, " %2d", *cp++);
	...
	if (anatype == 2) {
		...
		for (i = 0; i < 12; i++) {
			...
			for (cht1hh = chthh[i]; cht1hh->cnt > 0; cht1hh++) {
				fprintf(f1, "%4d [%2d|%2d|%2d|%2d|%2d](%2d) %4d %dh %2d:", cht1hh->cnt, cht1hh->wn[0],
				cht1hh->wn[1], cht1hh->wn[2], cht1hh->wn[3], cht1hh->wn[4], cht1hh->wn[5],
				cht1hh->gap, cht1hh->hit, cht1hh->encnt);
